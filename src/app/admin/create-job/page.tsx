        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 sm:p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2"><label className="text-sm text-zinc-300 mb-2 block">Assign To *</label>{personnelList.length === 0 ? <div className="p-4 bg-zinc-800 border border-zinc-700 rounded-lg text-sm text-zinc-400">No personnel. <Link href="/admin/personnel" className="underline text-white">Add personnel</Link></div> : <div className="border border-zinc-700 rounded-lg max-h-60 overflow-y-auto divide-y divide-zinc-800 bg-zinc-800">{personnelList.map(per => <label key={per.id} className={`flex items-center gap-3 p-3 cursor-pointer hover:bg-zinc-700 ${form.assignedPersonnel.includes(per.id) ? 'bg-zinc-700' : ''}`}><input type="checkbox" checked={form.assignedPersonnel.includes(per.id)} onChange={() => togglePer(per.id)} className="w-5 h-5" /><div><div className="text-sm text-white">{per.full_name}</div><div className="text-xs text-zinc-400">{per.email}</div></div></label>)}</div>}<p className="text-xs text-zinc-500 mt-2">{form.assignedPersonnel.length} selected</p></div>
            </div>
