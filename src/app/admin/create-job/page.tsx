...
export default function CreateJobPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 1. Create job doc - fast, this is all we need to redirect
      const jobRef = await addDoc(collection(db, 'jobCards'), jobData)
      const jobId = jobRef.id

      // 2. Create assignments in parallel using batch (super fast)
      setLoadingStep('Assigning personnel...')
      ...
      // 3. Instant redirect - don't wait for photos or email
      setLoadingStep('Job created! Uploading photos in background...')
      router.push(`/jobs/${jobId}?created=true`)

      // 4. Background tasks - don't block user (fire and forget)
      ...
      if (photos.length > 0) {
        Promise.allSettled(
          ...
          const photoUrls = results.map(r => r.status === 'fulfilled' ? r.value : null).filter(Boolean) as string[]
          // Send email after photos done (or with whatever succeeded)
        ...
        // No photos, just send email immediately in background
        const assignees = personnelList.filter(p => form.assignedPersonnel.includes(p.id))
    ...
    <div className="min-h-screen bg-[#0a0a0a]">
      ...
      <main className="max-w-4xl mx-auto px-4 py-6">
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 sm:p-8">
          ...
          <p className="text-sm text-zinc-400 mt-1 mb-6">Complete all fields to lodge job card • Photos compressed & uploaded in background for speed</p>
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2"><label className="text-sm text-zinc-300 mb-2 block">Photos (compressed automatically)</label><div className="border border-dashed border-zinc-700 rounded-lg p-8 text-center hover:border-zinc-600"><input type="file" multiple accept="image/*" onChange={handlePhotoChange} id="up" className="hidden" /><label htmlFor="up" className="cursor-pointer"><div className="text-2xl mb-1">📷</div><div className="text-sm text-zinc-300">Upload photos</div><div className="text-xs text-zinc-500 mt-1">Auto-compressed to ~200KB for fast upload</div></label></div>{photoPreviews.length > 0 && <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 mt-3">{photoPreviews.map((p,i) => <div key={i} className="relative"><img src={p} className="w-full h-20 object-cover rounded-lg border border-zinc-700" /><button type="button" onClick={() => removePhoto(i)} className="absolute -top-2 -right-2 w-6 h-6 bg-red-600 text-white rounded-full text-xs">✕</button></div>)}</div>}</div>
              <div className="md:col-span-2"><label className="text-sm text-zinc-300 mb-2 block">Assign To * (only confirmed)</label>{personnelList.length === 0 ? <div className="p-4 bg-zinc-800 border border-zinc-700 rounded-lg"><div className="text-sm text-white font-medium">No confirmed personnel</div><Link href="/admin/personnel" className="text-xs underline text-white mt-2 inline-block">Go to Personnel → Check pending</Link></div> : <div className="border border-zinc-700 rounded-lg max-h-60 overflow-y-auto divide-y divide-zinc-800 bg-zinc-800">{personnelList.map(per => <label key={per.id} className={`flex items-center gap-3 p-3 cursor-pointer hover:bg-zinc-700 ${form.assignedPersonnel.includes(per.id) ? 'bg-zinc-700' : ''}`}><input type="checkbox" checked={form.assignedPersonnel.includes(per.id)} onChange={() => togglePer(per.id)} className="w-5 h-5" /><div><div className="flex items-center gap-2"><span className="text-sm text-white">{per.full_name}</span><span className="text-[10px] bg-emerald-900/40 text-emerald-300 border border-emerald-800 px-1.5 py-0.5 rounded-full">Confirmed</span></div><div className="text-xs text-zinc-400">{per.email}</div></div></label>)}</div>}<p className="text-xs text-zinc-500 mt-2">{form.assignedPersonnel.length} selected</p></div>
            </div>
            <div className="flex gap-3 pt-6 border-t border-zinc-800"><Link href="/admin" className="px-6 py-3 border border-zinc-700 rounded-lg text-sm text-zinc-300 hover:bg-zinc-800">Cancel</Link><button disabled={loading} className="flex-1 bg-white text-black py-3 rounded-lg font-medium hover:bg-zinc-200 disabled:opacity-50 flex items-center justify-center gap-2">{loading ? <><div className="w-4 h-4 border-2 border-black border-t-transparent rounded-full animate-spin"></div>{loadingStep || 'Creating...'}</> : 'Create Job Card — Instant'}</button></div>
            {loading && <p className="text-xs text-zinc-400 text-center">Creating job instantly, photos & email will finish in background — you'll be redirected now</p>}
          </form>
        </div>
      </main>
    </div>
